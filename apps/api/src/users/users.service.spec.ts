...
describe('UsersService', () => {
  ...
  beforeEach(async () => {
    repo = {
      ...
      save: jest.fn(entity => Promise.resolve(Object.assign(new User(), entity))),
  ...
  describe('register', () => {
    it('saves a hashed password and strips sensitive data', async () => {
      ...
      expect(repo.create).toHaveBeenCalledWith(
        expect.objectContaining({ email: 'test@example.com', displayName: 'Tester', role: UserRole.STUDENT }),
      );
    ...
    it('rejects duplicate emails', async () => {
      repo.findOne.mockResolvedValueOnce(Object.assign(new User(), { id: 'existing', email: 'demo@example.com' }));

      await expect(
        service.register({ email: 'demo@example.com', displayName: 'Demo', password: 'ValidPass1!' }),
    ...
    it('rejects weak passwords', async () => {
      ...
      await expect(
        service.register({ email: 'weak@example.com', displayName: 'Weak', password: 'short' }),
  ...
  describe('login', () => {
    it('returns user when credentials are valid', async () => {
      ...
      expect(result).toMatchObject({ id: 'u1', email: 'user@example.com', displayName: 'User' });
      expect((result as any).passwordHash).toBeUndefined();
    });

    it('throws on invalid credentials', async () => {
      repo.findOne.mockResolvedValueOnce(null);
      await expect(service.login('missing@example.com', 'Whatever1!')).rejects.toThrow(UnauthorizedException);
    });
    ...
    it('throws when password does not match', async () => {
      ...
      await expect(service.login('user@example.com', 'Wrong1!')).rejects.toThrow(UnauthorizedException);
    });
  });

  describe('assignRole', () => {
    it('updates the role and hides password hash', async () => {
      const user = Object.assign(new User(), { id: 'u1', email: 'x@y', role: UserRole.STUDENT });
      ...
      expect(repo.save).toHaveBeenCalledWith(expect.objectContaining({ role: UserRole.ADMIN }));
  ...
  describe('updateProfile', () => {
    it('updates display name and password when valid', async () => {
      ...
      const result = await service.updateProfile('u1', { displayName: 'New Name', password: 'NewPass1!' });
    ...
    it('rejects invalid password updates', async () => {
      const user = Object.assign(new User(), { id: 'u1', email: 'user@example.com', displayName: 'Old' });
      repo.findOne.mockResolvedValueOnce(user);

      await expect(service.updateProfile('u1', { password: 'short' })).rejects.toThrow(BadRequestException);
    });
  });
  ...
  it('validatePasswordRules returns error list for weak password', () => {
    ...
    expect(outcome.errors).toEqual(expect.arrayContaining(['uppercase', 'digit', 'specialChar']));
  });
